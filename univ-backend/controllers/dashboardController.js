...
const getStats = async (req, res) => {
  try {
    ...
    // Répartition par filière (toutes inscriptions actives)
    const [par_filiere] = await db.query(
      `SELECT COALESCE(f.nom, 'Sans filière') AS filiere, COUNT(i.id) AS nb_etudiants
             ...
             WHERE i.statut IN ('actif', 'suspendu', 'diplome')
             GROUP BY f.id
             ORDER BY nb_etudiants DESC`,
    );

    // Répartition par niveau (toutes inscriptions actives)
    const [par_niveau] = await db.query(
      `SELECT niveau, COUNT(*) AS nb
             FROM inscriptions
             WHERE statut IN ('actif', 'suspendu', 'diplome')
             GROUP BY niveau
    ...
    const [derniers_inscrits] = await db.query(
      `SELECT COALESCE(e.matricule, i.id) AS matricule,
                    CONCAT(COALESCE(e.prenom,''),' ',COALESCE(e.nom,'')) AS nom_complet,
                    COALESCE(f.nom, 'Inconnue') AS filiere,
                    i.niveau, i.statut, i.date_inscription, i.id AS inscription_id,
             ...
             SELECT i2.id,
                    ...
                    END AS mention,
                    SUM(n.note * m.credit) / SUM(m.credit) AS moyenne,
                       GROUP_CONCAT(CONCAT(m.nom_matiere, ':', n.note) ORDER BY m.nom_matiere SEPARATOR ', ') AS notes_list
                FROM notes n
