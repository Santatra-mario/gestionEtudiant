...
const create = async (req, res) => {
  ...
  if (!niveauxValides.includes(niveau)) {
    return res
      .status(400)
      .json({ success: false, message: "Niveau invalide." });
  }
  ...
  try {
    ...
    if (exist.length > 0) {
      return res.status(409).json({
        success: false,
        message: "Cet étudiant est déjà inscrit à ce niveau pour cette année.",
      });
    }
...
const updateStatut = async (req, res) => {
  ...
  try {
    ...
    // Vérifier si l'erreur vient de l'ENUM MySQL
    if (err.code === "ER_DATA_TRUNCATED" || err.errno === 1265) {
      return res.status(400).json({
        success: false,
        message:
          "Statut invalide pour la base de données. Contactez l'administrateur pour ajouter 'transfere' à l'ENUM.",
      });
    }
    return res.status(500).json({ success: false, message: "Erreur serveur." });
  }
...
const remove = async (req, res) => {
  try {
    const [result] = await db.query("DELETE FROM inscriptions WHERE id = ?", [
      req.params.id,
    ]);
    if (result.affectedRows === 0)
