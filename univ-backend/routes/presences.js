...
router.post('/', async (req, res) => {
  try {
    const { inscription_id, matiere_id, date, statut, enregistre_par } = req.body;
    ...
    if (existing.length > 0) {
      ...
      return res.json({ message: 'Présence mise à jour avec succès', id: existing[0].id });
    }
    ...
    if (error.code === 'ER_NO_SUCH_TABLE') {
      errorMessage = 'La table presences n\'existe pas. Veuillez exécuter le script SQL.';
      statusCode = 503;
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      'DELETE FROM presences WHERE id = ?',
      [id]
    );
