...
import {
  PageHeader, Btn, Card, Modal, Input, Alert, Badge, Spinner
} from '../components/ui'
...
function MatiereModal({ filiereId, onClose, onSaved, initial }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (initial?.id) await api.put(`/filieres/matieres/${initial.id}`, payload)
  ...
  const semestreOptions = Array.from({ length: 10 }, (_, i) => `S${i + 1}`)
  ...
  return (
    <div>
      <PageHeader
        ...
        action={isAdmin && <Btn onClick={() => setModal('create')}>+ Nouvelle filière</Btn>}
      />
          ...
          {filieres.length === 0 && (
            <Card>
              <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '24px 0' }}>
                Aucune filière active. {isAdmin && 'Créez la première filière avec le bouton ci-dessus.'}
              </p>
            </Card>
          )}
            ...
            <Card key={f.id} style={{ padding: 0 }}>
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {isAdmin && (
                    <>
                      <Btn small variant="ghost" onClick={e => { e.stopPropagation(); setModal(f) }}>
                        Modifier
                      </Btn>
                      <Btn small variant="danger" onClick={e => {
                        e.stopPropagation()
                        openConfirm(
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {matieres[f.id].map(m => (
                        <div
                          ...
                          style={{
                            display: 'flex', justifyContent: 'space-between',
                            alignItems: 'center', background: 'var(--surface2)',
                        ...
                        >
                          ...
                          {isAdmin && (
                            <div style={{ display: 'flex', gap: 6 }}>
                              <Btn
                                small variant="ghost"
                                onClick={() => setMatiereModal({ filiereId: f.id, initial: m })}
                              >
                                Modifier
                              </Btn>
                              <Btn
