...
import {
  ...
  Building2, GraduationCap, User, FileText,
  RefreshCw, History, ToggleLeft, AlertCircle,
  ChevronDown, ChevronUp, CheckCircle2, Bell,
} from "lucide-react";
  ...
  useEffect(() => {
    const onAccepte = (e) => {
      ...
      setNotifs(prev => [{
        ...
        message: "Le transfert a été accepté par l'administrateur. L'étudiant a été retiré de la liste.",
    ...
    const onRefuse = (e) => {
      ...
      setNotifs(prev => [{
        ...
        titre: `${etudiantPrenom} ${etudiantNom}`,
        message: "La demande de transfert a été refusée par l'administrateur.",
        heure: new Date().toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }),
        date: new Date().toLocaleDateString("fr-FR"),
        lu: false,
      }, ...prev].slice(0, 20));
      ...
      */}
      <div
        ...
        style={{
          position: "fixed",
          top: 13,
          right: 210,          /* ← décalé à gauche du badge rôle */
          zIndex: 1000,
        }}
      >
        {/* ── Bouton cloche ── */}
        <button
          onClick={() => { setOpen(v => !v); if (!open) marquerToutesLues(); }}
        ...
        >
          ...
          <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Bell
              size={18}
              style={{
                color: open ? "var(--accent)" : nonLues > 0 ? "#a78bfa" : "var(--text-muted)",
        ...
        {open && (
          <div
            ...
            style={{
              ...
              borderRadius: 16,
              boxShadow: "0 16px 48px rgba(0,0,0,0.4), 0 2px 12px rgba(0,0,0,0.2)",
              display: "flex",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{
                  ...
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"
                      fill="rgba(99,102,241,0.2)" stroke="#818cf8" strokeWidth="2"
                      strokeLinecap="round" strokeLinejoin="round"/>
                    <polyline points="14,2 14,8 20,8" fill="none" stroke="#818cf8"
                      strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    <line x1="8" y1="13" x2="16" y2="13" stroke="#818cf8"
                      strokeWidth="2" strokeLinecap="round"/>
                    <polyline points="13,10 16,13 13,16" fill="none" stroke="#818cf8"
                      strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
            ...
            {notifs.length === 0 ? (
              <div style={{
                ...
                <div style={{
                  ...
                  <Bell size={24} style={{ color: "var(--border)", opacity: 0.5 }} />
                </div>
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-muted)", opacity: 0.7 }}>
                    Les transferts acceptés ou refusés<br/>apparaîtront ici
                  </div>
                </div>
              </div>
            ) : (
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
function StudentSearchSelect({ etudiants, value, onChange, label = "Étudiant *" }) {
  ...
  const filtered = etudiants.filter(e => {
    ...
    return `${e.prenom} ${e.nom} ${e.matricule}`.toLowerCase().includes(query.toLowerCase());
  });
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative" }}>
      ...
      <div style={{ position: "relative" }}>
        <Search size={14} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)", pointerEvents: "none", zIndex: 1 }} />
        <input
          value={displayValue}
        ...
        {(value || query) && (
          <button onClick={handleClear}
            style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "transparent", border: "none", cursor: "pointer", color: "var(--text-muted)", display: "flex", padding: 2, borderRadius: 4 }}
              ...
              {filtered.map(e => (
                <div key={e.id}
                  ...
                  style={{
                    ...
                    borderBottom: "1px solid var(--border)",
                    background: String(e.id) === String(value) ? "var(--accent-glow)" : "transparent",
                  }}
                  onMouseEnter={ev => ev.currentTarget.style.background = "var(--surface2)"}
                  onMouseLeave={ev => ev.currentTarget.style.background = String(e.id) === String(value) ? "var(--accent-glow)" : "transparent"}
                >
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, var(--accent), var(--accent-light, #818cf8))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1, overflow: "hidden" }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "monospace" }}>
                      {e.matricule}
                      {e.filiere_nom && <span style={{ marginLeft: 8, color: "var(--accent)" }}>{e.filiere_nom} · {e.niveau}</span>}
                    </div>
                  </div>
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
function FiliereSelect({ filieres, value, onChange, label = "Filière destination *", placeholder = "-- Choisir une filière --" }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {label && (
        <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block" }}>
          {label}
        </label>
      )}
      <select
  ...
  return (
    <Modal title="Nouvelle demande de transfert" onClose={onClose} width={620}>
      <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: "8px 0" }}>
        ...
        <FormSection title="Destination dans notre établissement" icon={GraduationCap}>
          <FormRow>
            <FiliereSelect
              filieres={filieres}
              value={form.filiere_destination_id}
              onChange={id => setForm(f => ({ ...f, filiere_destination_id: id }))}
            />
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>Niveau *</label>
              <select
                value={form.niveau}
                onChange={e => setForm(f => ({ ...f, niveau: e.target.value }))}
                style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: "1.5px solid var(--accent)", background: "var(--surface)", color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "var(--font-body)", cursor: "pointer" }}
              >
        ...
        <FormSection title="Motif" icon={FileText}>
          <textarea value={form.motif} onChange={set("motif")}
            ...
            style={{ width: "100%", minHeight: 80, padding: "10px 14px", borderRadius: 8, border: "1.5px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 14, resize: "vertical", boxSizing: "border-box" }}
          />
        </FormSection>
...
function ChangementFiliereModal({ onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    nouvelle_filiere_id: "", nouveau_niveau: "", annee_universitaire: "2026-2027",
    motif: ""
  });
  ...
  const handleSelectEtudiant = async (etudiantId) => {
    setForm(f => ({ ...f, etudiant_id: etudiantId, inscription_id: "", nouvelle_filiere_id: "", nouveau_niveau: "" }));
    if (!etudiantId) { setInscriptions([]); return; }
  ...
  const isValid = form.etudiant_id && form.inscription_id &&
                  form.nouvelle_filiere_id && form.nouveau_niveau && form.annee_universitaire;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess("Changement effectué avec succès. L'ancienne inscription est archivée.");
    ...
    <Modal title="Changement de filière / niveau" onClose={onClose} width={620}>
      <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: "8px 0" }}>
        <Alert type="warning" style={{ margin: 0 }}>
          Cette action <strong>archive l'inscription actuelle</strong> (statut → abandonné) et
          crée une <strong>nouvelle inscription active</strong> dans la filière choisie.
        </Alert>
        {error   && <Alert type="danger">{error}</Alert>}
        ...
        {form.etudiant_id && (
          <FormSection title="Inscription actuelle à archiver" icon={History}>
            {loadingInscr ? (
              ...
              <Alert type="warning">Aucune inscription active trouvée pour cet étudiant.</Alert>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>Inscription active *</label>
                <select value={form.inscription_id} onChange={e => setForm(f => ({ ...f, inscription_id: e.target.value }))}
                  style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${form.inscription_id ? "var(--accent)" : "var(--border)"}`, background: "var(--surface)", color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "var(--font-body)" }}>
                  <option value="">-- Choisir l'inscription à changer --</option>
                  {inscriptions.map(i => (
                    <option key={i.id} value={i.id}>
                      {i.filiere_nom || `Filière #${i.filiere_id}`} — {i.niveau} — {i.annee_universitaire}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <FormSection title="Nouvelle destination" icon={GraduationCap}>
            <FormRow>
              <FiliereSelect
                ...
                onChange={id => setForm(f => ({ ...f, nouvelle_filiere_id: id }))}
                label="Nouvelle filière *"
              />
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>Nouveau niveau *</label>
                <select
                  value={form.nouveau_niveau}
                  onChange={e => setForm(f => ({ ...f, nouveau_niveau: e.target.value }))}
                  style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${form.nouveau_niveau ? "var(--accent)" : "var(--border)"}`, background: "var(--surface)", color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "var(--font-body)", cursor: "pointer" }}
                >
        ...
        <FormSection title="Motif du changement" icon={FileText}>
          <textarea value={form.motif} onChange={set("motif")}
            ...
            style={{ width: "100%", minHeight: 70, padding: "10px 14px", borderRadius: 8, border: "1.5px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 14, resize: "vertical", boxSizing: "border-box" }}
          />
        </FormSection>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 16, borderTop: "1px solid var(--border)" }}>
          <Btn variant="ghost" onClick={onClose} icon={<X size={15}/>}>Annuler</Btn>
          <Btn onClick={handleSubmit} loading={loading} disabled={!isValid}
            icon={<RefreshCw size={15}/>}>Effectuer le changement</Btn>
        </div>
      </div>
    </Modal>
  );
...
function ChangementStatutModal({ onClose, onSaved }) {
  ...
  const [error, setError]                 = useState("");
  const [form, setForm] = useState({
    etudiant_id: "", inscription_id: "", nouveau_statut: "", motif: ""
  });
  ...
  const handleSelectEtudiant = async (etudiantId) => {
    setForm(f => ({ ...f, etudiant_id: etudiantId, inscription_id: "", nouveau_statut: "" }));
    if (!etudiantId) { setInscriptions([]); return; }
    setLoadingInscr(true);
    try {
      const { data } = await api.get(`/inscriptions?etudiant_id=${etudiantId}`);
  ...
  const inscriptionSelectionnee = inscriptions.find(i => String(i.id) === String(form.inscription_id));
  const isValid = form.inscription_id && form.nouveau_statut &&
  ...
  const statutColors = { actif: "success", suspendu: "warning", diplome: "accent", abandonne: "danger" };
 
  return (
    <Modal title="Changer le statut d'une inscription" onClose={onClose} width={560}>
      <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: "8px 0" }}>
        {error && <Alert type="danger">{error}</Alert>}
        <FormSection title="Étudiant" icon={User}>
          <StudentSearchSelect
        ...
        {form.etudiant_id && (
          <FormSection title="Inscription à modifier" icon={ClipboardIcon}>
            {loadingInscr ? (
              ...
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>Inscription *</label>
                  <select value={form.inscription_id} onChange={e => setForm(f => ({ ...f, inscription_id: e.target.value }))}
                    style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${form.inscription_id ? "var(--accent)" : "var(--border)"}`, background: "var(--surface)", color: "var(--text)", fontSize: 14, outline: "none", fontFamily: "var(--font-body)" }}>
                    <option value="">-- Choisir une inscription --</option>
                    {inscriptions.map(i => (
                      <option key={i.id} value={i.id}>
                        {i.filiere_nom || `Filière #${i.filiere_id}`} — {i.niveau} — {i.annee_universitaire} [{i.statut}]
                      </option>
                    ))}
                  </select>
                </div>
                {inscriptionSelectionnee && (
                  <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 8, background: "var(--surface2)", border: "1px solid var(--border)" }}>
                    <span style={{ fontSize: 13, color: "var(--text-muted)" }}>Statut actuel :</span>
                    <Badge variant={statutColors[inscriptionSelectionnee.statut] || "default"}>
                      {inscriptionSelectionnee.statut}
                    </Badge>
                  </div>
                )}
                {form.inscription_id && (
                  <Select label="Nouveau statut *" value={form.nouveau_statut} onChange={set("nouveau_statut")}
                    style={{ marginTop: 10 }}>
                    <option value="">-- Choisir --</option>
                    {STATUT_INSCRIPTION
                      .filter(s => s !== inscriptionSelectionnee?.statut)
        ...
        <FormSection title="Motif (optionnel)" icon={FileText}>
          <textarea value={form.motif} onChange={set("motif")}
            ...
            style={{ width: "100%", minHeight: 60, padding: "10px 14px", borderRadius: 8, border: "1.5px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 14, resize: "vertical", boxSizing: "border-box" }}
          />
        </FormSection>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 16, borderTop: "1px solid var(--border)" }}>
          <Btn variant="ghost" onClick={onClose} icon={<X size={15}/>}>Annuler</Btn>
          <Btn onClick={handleSubmit} loading={loading} disabled={!isValid}
            icon={<ToggleLeft size={15}/>}>Appliquer le changement</Btn>
        </div>
      </div>
    </Modal>
  );
  ...
  return (
    <Modal title="Historique des inscriptions" onClose={onClose} width={700}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "8px 0" }}>
        ...
        {!selectedId && (
          <EmptyState icon={History} title="Sélectionner un étudiant"
          ...
          <>
            {etudiantChoisi && (
              <div style={{ padding: "10px 14px", borderRadius: 10, background: "var(--surface2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg, var(--accent), var(--accent-light))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                  {`${etudiantChoisi.prenom?.[0] || ""}${etudiantChoisi.nom?.[0] || ""}`.toUpperCase()}
                </div>
                <div>
                  <div style={{ fontWeight: 600, color: "var(--text)" }}>{etudiantChoisi.prenom} {etudiantChoisi.nom}</div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "monospace" }}>{etudiantChoisi.matricule}</div>
                </div>
            ...
            {inscriptions.length === 0 ? (
              <EmptyState icon={History} title="Aucune inscription"
                description="Cet étudiant n'a aucune inscription enregistrée." />
            ) : (
              <div style={{ background: "var(--surface)", borderRadius: 10, border: "1px solid var(--border)", overflow: "hidden" }}>
                <Table headers={["Filière", "Niveau", "Année", "Statut", "Date inscription"]}>
                  {inscriptions.map(i => (
                    <Tr key={i.id}>
                      <Td><span style={{ fontWeight: 500 }}>{i.filiere_nom || `#${i.filiere_id}`}</span></Td>
                      <Td><Badge variant="accent">{i.niveau}</Badge></Td>
                      <Td>{i.annee_universitaire}</Td>
                      <Td><Badge variant={statutColors[i.statut] || "default"}>{i.statut}</Badge></Td>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "8px 0" }}>
        ...
        <div>
          ...
          <textarea value={motif} onChange={e => setMotif(e.target.value)}
            ...
            style={{ width: "100%", minHeight: 100, padding: "10px 14px", borderRadius: 8, border: "1.5px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 14, resize: "vertical", boxSizing: "border-box" }}
          />
        </div>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 16, borderTop: "1px solid var(--border)" }}>
...
function AnnulationModal({ transfert, onClose, onSaved }) {
  ...
  const handleAnnuler = async () => {
    ...
    setError("");
    try {
      await api.put(`/transferts/${transfert.id}/annuler`, { motif_annulation: motif });
      onSaved();
    } catch (err) {
  ...
  return (
    <Modal title="Annuler / Corriger ce transfert" onClose={onClose} width={480}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "8px 0" }}>
        <Alert type="danger">
          Action <strong>administrateur uniquement</strong>. L'annulation remet le transfert à l'état
          « en attente » pour correction.
        </Alert>
        {error && <Alert type="danger">{error}</Alert>}
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 16, borderTop: "1px solid var(--border)" }}>
          <Btn variant="ghost" onClick={onClose}>Annuler</Btn>
          <Btn variant="danger" onClick={handleAnnuler} loading={loading}
            icon={<AlertCircle size={15}/>}>Confirmer l'annulation</Btn>
        </div>
      </div>
    </Modal>
  );
  ...
  const handleAccepter = async (id, etudiantPrenom, etudiantNom) => {
    ...
    try {
      ...
      if (err.response?.status === 409) {
        notify.warning(`⚠️ Ce transfert a déjà été traité par un autre administrateur.`);
  ...
  return (
    <>
      ...
      <div className="page-enter" style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        ...
        <PageHeader
          ...
          action={canEdit && (
            <div style={{ display: "flex", gap: 8, alignItems: "center", position: "relative" }}>
              ...
              <div style={{ position: "relative" }}>
                <Btn variant="ghost" onClick={() => setShowActions(v => !v)}
                  icon={showActions ? <ChevronUp size={15}/> : <ChevronDown size={15}/>}>
                  Autres actions
                </Btn>
                {showActions && (
                  <>
                    <div onClick={() => setShowActions(false)} style={{ position: "fixed", inset: 0, zIndex: 199 }} />
                    <div style={{
                      position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 200,
                      background: "var(--surface)", border: "1.5px solid var(--border)",
                      borderRadius: 12, boxShadow: "var(--shadow-modal)", minWidth: 230,
                      overflow: "hidden", animation: "fadeIn 0.12s ease",
                    }}>
                      {[
                        { icon: RefreshCw, label: "Changer filière / niveau", action: () => { setShowActions(false); setModal("changer_filiere"); } },
                        { icon: ToggleLeft, label: "Changer statut inscription", action: () => { setShowActions(false); setModal("changer_statut"); } },
                        { icon: History, label: "Voir historique étudiant", action: () => { setShowActions(false); setModal("historique"); } },
                      ].map(({ icon: Icon, label, action }) => (
                        <button key={label} onClick={action}
                          style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "11px 16px", background: "transparent", border: "none", cursor: "pointer", color: "var(--text)", fontSize: 14, textAlign: "left", transition: "background 0.12s", fontFamily: "var(--font-body)" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 28 }}>
          {[
            ...
            { label: "En attente", value: stats.en_attente, color: "var(--warning)", bg: "rgba(245,158,11,0.1)" },
            { label: "Acceptés", value: stats.accepte, color: "var(--success)", bg: "rgba(34,197,94,0.1)" },
            { label: "Refusés", value: stats.refuse, color: "var(--danger)", bg: "rgba(239,68,68,0.1)" },
          ].map(({ label, value, color, bg }) => (
            <div key={label} style={{ padding: "16px 20px", background: "var(--surface)", border: `1.5px solid ${color}30`, borderRadius: 12, boxShadow: "var(--shadow-sm)" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative", flex: "1 1 300px", maxWidth: 460 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              style={{ width: "100%", boxSizing: "border-box", background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 40, color: "var(--text)", padding: "11px 18px 11px 44px", fontSize: 14, outline: "none" }}
            />
          </div>
            ...
            {["tous", "en_attente", "accepte", "refuse"].map(s => (
              <button key={s}
                ...
                style={{
                  ...
                  fontSize: 13, cursor: "pointer", fontFamily: "var(--font-body)", transition: "all 0.15s",
                ...
                {s !== "tous" && (
                  <span style={{ marginLeft: 5, opacity: 0.8 }}>({stats[s] || 0})</span>
                )}
              </button>
            ))}
          </div>
        ...
        {/* ── Tableau ── */}
        <div style={{ background: "var(--surface)", borderRadius: "var(--radius-lg)", border: "2px solid var(--border)", overflow: "hidden", boxShadow: "var(--shadow)" }}>
          <Table headers={["Matricule", "Étudiant", "Origine", "Destination", "Niveau", "Année", "Statut", "Actions"]}>
            {loading ? (
              ...
              <tr><td colSpan={8}>
                <EmptyState icon={ArrowLeftRight} title="Aucun transfert"
                  description={search || filtreStatut !== "tous" ? "Aucun résultat pour ces filtres." : "Aucune demande de transfert pour le moment."} />
              </td></tr>
            ) : filtered.map(t => {
                  ...
                  <Td>
                        ...
                        {isAdmin && (
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            <Btn small variant="success"
                              icon={isProcessing ? <Spinner size={12}/> : <Check size={13}/>}
                              disabled={isProcessing}
                              onClick={() => handleAccepter(t.id, t.etudiant_prenom, t.etudiant_nom)}>
                    ...
                    {t.statut !== "en_attente" && (
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        ...
                        {isAdmin && (
                          <Btn small variant="ghost" icon={<AlertCircle size={12}/>}
        ...
        {user?.role === "secretaire" && (
          <div style={{ marginTop: 16, padding: "10px 16px", borderRadius: 10, background: "rgba(34,197,94,0.07)", border: "1px solid rgba(34,197,94,0.2)", display: "flex", alignItems: "center", gap: 8 }}>
            <AlertCircle size={15} style={{ color: "var(--success)", flexShrink: 0 }} />
            <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
              En tant que secrétaire, vous pouvez initier des demandes de refus et changer filière/statut, mais <strong>seul un administrateur peut accepter</strong> un transfert inter-établissement.
            </span>
          </div>
        )}
      </div>
      ...
      {modal === "changer_filiere" && (
        <ChangementFiliereModal onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
      {modal === "changer_statut" && (
        <ChangementStatutModal onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
      {modal === "historique" && (
      ...
      {annulModal && (
        <AnnulationModal transfert={annulModal}
          onClose={() => setAnnulModal(null)}
          onSaved={() => { setAnnulModal(null); load(); }} />
      )}
    </>
  );
}
