...
const mentionColor = { Admis: "success", Rattrapage: "warning", "Ajourné": "danger" };
...
// L'API retourne { success: true, data: [...] }  → on lit .data
...
export default function NotesSaisiePage() {
  ...
  // matieres = liste depuis GET /filieres/:id/matieres  (FilièresPage)
  const [matieres, setMatieres]   = useState([]);
  // bulletin = { inscription, bulletin: { S1: {notes, moyenne, mention}, S2: ... } }
  const [bulletin, setBulletin]   = useState(null);
  // editNotes = { "matiere_id": "valeur_string" }
  ...
  const [confirmState, setConfirmState] = useState({
    ...
    noteId: null, matiereId: null, loading: false,
  });

  const openConfirm = (noteId, matiereId, message) =>
    setConfirmState({ open: true, title: "Suppression", message, noteId, matiereId, loading: false });
  ...
  // ── Chargement matières (FilièresPage) + bulletin (notes existantes) ─────────
  const loadData = useCallback(async (inscriptionId) => {
    if (!inscriptionId) {
      setBulletin(null); setMatieres([]); setEditNotes({});
      return;
    }
    ...
    const insc = inscriptions.find((i) => String(i.id) === String(inscriptionId));

    try {
      // ── ÉTAPE 1 : Matières de la filière depuis FilièresPage ──────────────
      // GET /filieres/:filiereId/matieres → { success: true, data: [...] }
      // IMPORTANT : on lit responseData.data (pas responseData directement)
      let matList = [];
      if (insc?.filiere_id) {
        try {
      ...
      // ── ÉTAPE 2 : Bulletin (notes déjà enregistrées) ──────────────────────
      // GET /notes/bulletin/:id → { success, inscription, bulletin: { S1: {...}, S2: {...} } }
      let bData = { inscription: insc ?? {}, bulletin: {} };
      try {
        const bRes = await api.get(`/notes/bulletin/${inscriptionId}`);
        bData = bRes.data; // { inscription, bulletin }
      } catch (e) {
        // Pas encore de notes = normal pour un nouvel étudiant
        console.info("Aucune note existante pour cette inscription.");
      }
      setBulletin(bData);

      // ── ÉTAPE 3 : Initialiser editNotes ───────────────────────────────────
      // Toutes les matières de la filière → valeur "" (vide)
      const init = {};
      matList.forEach((m) => { init[String(m.id)] = ""; });

      // Écraser avec les notes déjà enregistrées dans le bulletin
      ...
      setMsg({ text: "Impossible de charger les données de cette inscription.", type: "danger" });
    } finally {
      setLoading(false);
    }
  }, [inscriptions]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (notes.length === 0) {
        setMsg({ text: "Aucune note valide (0–20) à enregistrer.", type: "danger" });
        setSaving(false);
        return;
      }
      ...
      const updated = {};
      matieres.forEach((m) => { updated[String(m.id)] = ""; });
      Object.values(r.data?.bulletin ?? {}).forEach((sem) => {
        (sem.notes ?? []).forEach((n) => {
          if (n.matiere_id != null) updated[String(n.matiere_id)] = String(parseFloat(n.note));
        });
      });
  ...
  const canEdit   = ["administrateur", "secretaire", "enseignant"].includes(user?.role);
  ...
  // Chercher une note existante dans le bulletin pour une matière donnée
  const getNoteExistante = (matiereId) => {
    if (!bulletin) return null;
    for (const sem of Object.values(bulletin.bulletin || {})) {
      const found = (sem.notes || []).find((n) => String(n.matiere_id) === String(matiereId));
  ...
  const renderSaisieTable = (sem, list) => {
    ...
    return (
      <Card key={sem}>
        {/* En-tête semestre + moyenne estimée */}
        <div style={{
          ...
          {moyenneTemp && (
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
                Moyenne :{" "}
                <strong style={{
                  color: parseFloat(moyenneTemp) >= 10 ? "var(--success)" : "var(--danger)",
                  fontSize: 15,
                }}>
                  {moyenneTemp}/20
                </strong>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          <thead>
            <tr>
              {[
                "Matière", "Code", "Coeff.", "Note /20", "Pondérée",
                ...(canDelete ? ["Action"] : []),
              ].map((h) => (
                <th key={h} style={{
          ...
          <tbody>
            {list.map((m) => {
              ...
              const isInvalid     = valSaisie !== "" && (isNaN(valNum) || valNum < 0 || valNum > 20);
              const pond          = (!isNaN(valNum) && valNum >= 0 && valNum <= 20)
                ? (valNum * parseFloat(m.coefficient)).toFixed(2)
                : "—";
                    ...
                    {canEdit ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        <input
                          ...
                          style={{
                            ...inputNoteStyle,
                            borderColor: isInvalid ? "var(--danger)" : "var(--border)",
                          }}
                        />
                        {isInvalid && (
                          <span style={{ fontSize: 11, color: "var(--danger)" }}>0 – 20</span>
                        )}
                      </div>
                    ) : (
                        ...
                        <Btn
                          ...
                          onClick={() =>
                            openConfirm(
                              noteExistante.note_id,
                              m.id,
                              `Supprimer la note de "${m.nom}" ? Cette action est irréversible.`
                            )
                          }
                        >
    ...
    <div>
          ...
          <div style={{ flex: 1, minWidth: 220 }}>
            ...
            <div style={{ position: "relative" }}>
              <input
                ...
                style={{ ...selectStyle, padding: "10px 13px 10px 36px", color: "var(--text)" }}
                onFocus={(e) => {
                  e.currentTarget.style.borderColor = "var(--accent)";
                  e.currentTarget.style.boxShadow = "0 0 0 3px rgba(99,102,241,0.18)";
                }}
      ...
      <Card style={{ marginBottom: 20 }}>
        {inscLoading ? (
          ...
          <NativeSelect
            label={`Sélectionner une inscription${inscriptions.length > 0
          ...
          >
            ...
            {filteredInscriptions.length === 0
              ...
              : filteredInscriptions.map((i) => (
                <option key={i.id} value={String(i.id)}>{inscriptionLabel(i)}</option>
              ))
            }
          </NativeSelect>
        )}
      ...
      {!loading && selectedInscription && bulletin && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ marginLeft: "auto" }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text)" }}>
                  {matieres.length > 0
                    ...
                    : <span style={{ color: "var(--danger)" }}>Aucune matière</span>
                  }
                </div>
              </div>
            </div>
          ...
          {/* Tableaux de saisie par semestre (matières de FilièresPage) */}
          {matieres.length === 0 ? (
            <Card>
          ...
          {matieres.length > 0 && (
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, padding: "4px 0" }}>
              {canEdit && (
                <Btn onClick={handleSave} disabled={saving || nbNotesRemplies === 0}>
      ...
      {!selectedInscription && !loading && (
        <Card>
          <p style={{
            ...
            Sélectionnez une inscription ci-dessus pour consulter ou saisir les notes.
          </p>
        </Card>
      )}
